...
class VLCTube(tk.Tk):
	...
	def start_thread(self, event=None):
		...
		if query and not query.isspace():
			search_thread = threading.Thread(target=self.get_url, args=(query,))
	...
	def get_url(self, query):
		""" Uses the youtubesearchpython package to search Youtube 
			and get the stream url
		"""
		try:
			vid_result = ysp.VideosSearch(f"{query}", limit=1)
			vid_info = vid_result.result()["result"][0]
			vid_url = vid_info["link"]
